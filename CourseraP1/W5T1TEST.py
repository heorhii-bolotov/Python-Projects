"""
    �­то unittest для тестирования вашего класса Client из задания на неделе 5.
    ...
    �­то не обязательное требование, если вы назвали мобуль по-другому, то
...
class ServerSocket:
    ...
    def sendall(self, answer):
        return self.send(answer)

    def send(self, answer):
        if answer in self.rsp_map:
            self.response_buf.append(self.rsp_map[answer])
        else:
            raise ServerSocketException(f"запрос не соответствует протоколу: {answer}")
